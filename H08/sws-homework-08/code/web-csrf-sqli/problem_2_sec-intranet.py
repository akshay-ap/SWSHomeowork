...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if action == 'login':
            ...
            sql = f"SELECT username FROM users WHERE username = ? AND password = ?"
...
class MyHandler(MyHandler):
    ...
    def do_GET(self):
        ...
        sql = f"SELECT username FROM users WHERE username = ? AND privileges = ?"
        print (f"Executing SQL: {sql}")
        res = connection.execute(sql,(username,'all'))
        entry = res.fetchone()
        if entry is None:
    ...
    def do_POST(self):
        ...
        if self.path == "/admin/add":
            ...
            print (f"Executing SQL: {sql}")
            res = connection.execute(sql,(randint(100,200),name,password,date_str,'user'))
            connection.commit()
            print("res:", res)
            self.redirect('/admin')
